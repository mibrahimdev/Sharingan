...
internal class PersistenceControllerTest {
    private fun event(id: String): EventRow =
        EventRow(
            rawId = id,
            timestampMillis = 0L,
            type = "HTTP",
    ...
    @Test
    fun `Given a burst within the channel capacity When flushed Then every event is persisted`() =
        runBlocking {
            val total = 500
    ...
    @Test
    fun `Given a burst beyond the channel capacity When flushed Then the newest events survive`() =
        runBlocking {
            val driver = createTestDriver()
            val controller =
                PersistenceController<EventRow>(
                    toRow = { it },
                    driver = driver,
                    batchSize = 4,
            ...
            // Submit before start() so the flusher cannot drain mid-burst; DROP_OLDEST must retain the newest 4.
    ...
    @Test
    fun `Given a toRow mapper that throws on one event When flushed Then the flusher survives and other events persist`() =
        runBlocking {
            val driver = createTestDriver()
            val controller =
                PersistenceController<EventRow>(
    ...
    @Test
    fun `Given many events When flushed Then they are written in batches not one per event`() =
        runBlocking {
            val total = 200
            val batchSize = 50
            val driver = createTestDriver()
    ...
    @Test
    fun `Given two sessions When events reuse a raw id Then persisted ids do not collide`() =
        runBlocking {
            val driver = createTestDriver()
    ...
    @Test
    fun `Given a pending batch When clear is called Then no events are resurrected`() =
        runBlocking {
            val driver = createTestDriver()
            val controller =
                PersistenceController<EventRow>(
                    ...
                    driver = driver,
                    batchSize = 100,
                    // Short deadline so flushed-1 lands; pending events queue behind ClearAll well before the next deadline.
    ...
    @Test
    fun `Given a bounded channel When filled beyond capacity Then oldest are dropped and newest survive`() =
        runBlocking {
    ...
    @Test
    fun `Given stop is called twice Then it is idempotent and does not crash`() =
        runBlocking {
            val driver = createTestDriver()
            val controller =
                PersistenceController<EventRow>(
    ...
    @Test
    fun `Given start is called twice without stop Then only one flusher runs and every event is persisted`() =
        runBlocking {
            val total = 5
            val driver = createTestDriver()
            val controller =
    ...
    @Test
    fun `Given stop has been called When start is called again Then it throws IllegalStateException`() =
        runBlocking {
            val driver = createTestDriver()
            val controller = PersistenceController<EventRow>(toRow = { it }, driver = driver)
            controller.start()
    ...
    @Test
    fun `Given buffered events When stopped Then the pending batch is drained before close`() =
        runBlocking {
            val driver = createTestDriver()
            val controller =
                PersistenceController<EventRow>(
    ...
    @Test
    fun `Given events under the batch size When the deadline passes Then a deadline flush lands`() =
        runBlocking {
            val driver = createTestDriver()
            val controller =
                PersistenceController<EventRow>(
    ...
    @Test
    fun `Given a slow steady stream When events arrive under the batch size Then a flush lands within the interval`() =
        runBlocking {
            val driver = createTestDriver()
            val controller =
                PersistenceController<EventRow>(
