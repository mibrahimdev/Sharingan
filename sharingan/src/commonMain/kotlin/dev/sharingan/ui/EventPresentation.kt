...
internal fun SharinganColors.httpStatusTint(status: Int?): Tint =
    when {
...
internal fun SharinganColors.httpMethodTint(method: String): Tint =
...
internal fun SharinganColors.mqttDirectionTint(direction: MqttDirection): Tint =
    when (direction) {
...
internal fun SharinganColors.bleOperationTint(operation: BleOperation): Tint =
    when (operation) {
...
internal fun presentationOf(
    colors: SharinganColors,
    event: SharinganEvent,
): EventPresentation = descriptorOf(event).presentation(colors, event)
...
internal fun rowTime(clockTime: String): String = if (clockTime.length >= 8) clockTime.substring(3, 8) else clockTime
