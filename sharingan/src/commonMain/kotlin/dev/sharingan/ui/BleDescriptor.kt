...
internal object BleDescriptor : ProtocolDescriptor<BleEvent>() {
    ...
    override fun searchHaystack(event: BleEvent): List<String?> =
        listOf(
    ...
    override fun ticker(event: BleEvent): String = "${event.operation.name} ${event.characteristic ?: event.device}"

    override fun markdown(event: BleEvent): String =
        buildString {
    ...
    override fun summary(event: BleEvent): String = "${event.operation.name} ${event.characteristic ?: "—"} (${event.device})"

    override fun fields(event: BleEvent): List<Pair<String, String>> =
        buildList {
